import numpy as np
""" 
Python translation of original distclust_fast.m file
...
def distclust(dists, nsam, expo=-2, ifresamp=0, iftrump=1):
    ...
    if iftrump == 0:
       for i in range(ncla):
          if isinstance(expo, str):
             ...
             for j in range(samstart[i], samend[i]):
                 ...
                 av[i, j] = (np.sum(np.concatenate([dists[samstart[i]:j, j], dists[j+1:samend[i], j]]) ** expo) / (nsam[i] - 1)) ** (1 / expo)
    else:
       ...
       for i in range(ncla):
          ...
          temp[nz] = 1
          # av[i, :] = -np.sum(temp) / nsm # ..doesnt account for column-major indexing in matlab
          # av: rows = classes, columns = spike - trains
    ...
    return anear.astype(int) # prosper, prosper, prosper and be in good health